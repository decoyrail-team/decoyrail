...
In short, same-user mode guards against accidents and prompt-driven
...
agent. Traffic that ignores the proxy simply carries the decoy (never a
...
itself: it covers tools that behave normally, not an adversary with full
local access.
...
detection in depth; decoys are honeytokens, not a guaranteed exfil block.
...
Names, addresses, and free-text health or personal data need judgment, not
rules, and wait for the local judge model. Decoyrail makes no HIPAA claim.
