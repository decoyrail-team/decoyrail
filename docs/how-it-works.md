...
3. **Decoys are honeytokens.** A decoy observed heading anywhere the winning
   ...
   The request is blocked and the event is recorded: detection, not just
   prevention.
...
and go through the same swap/tripwire pipeline. Recognized provider keys
carry a provider label the default policy releases at that provider's API,
so they keep working; everything else is tripwire-only. `--pass-env` and
`--pass-all-env` opt out. See
...
already in flight. This is telemetry, not prevention. SSE streams skip
