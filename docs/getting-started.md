...
Recognized provider keys stay usable: they carry a
provider label (`anthropic`, `openai`, `github`, `gitlab`, `slack`, `npm`),
and the default policy's provider rules release those labels at that
provider's API. Everything else becomes a tripwire: the child cannot use
the credential, and any attempt to send it is blocked and recorded. To make
one usable, release it by its session name (`env:` plus
