...
- **Accurate spend metering** (free, shipped): the proxy parses the token
  ...
  per-request bill. Its tokens are still metered in full, because a plan is
  not free tokens: the included allowance is finite, heavy sessions hit plan
  limits, and usage beyond the plan bills at API rates. The reference-cost
  work below builds on this. Token counts also land in the audit log per
...
- **Canary decoys.** Opt-in provisioning of a decoy as a canarytoken
  ...
  machine, not just through the proxy. Decoyrail itself never calls a canary
